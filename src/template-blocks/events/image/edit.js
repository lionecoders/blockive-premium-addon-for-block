...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: `bpafb-tb-event-image bpafb-fi-hover-${ hoverEffect || 'none' }${ overlayColor ? ' bpafb-has-overlay' : '' }`,
		style: {
		...
		<>
			...
			<figure { ...blockProps }>
				{ previewImageUrl ? (
					...
					<div className="bpafb-tb-event-image-placeholder">
						{ isResolving
							? __( 'Loading…', 'blockive-premium-addon-for-block' )
							: __( 'Event Image Placeholder', 'blockive-premium-addon-for-block' ) }
					</div>
				) }
