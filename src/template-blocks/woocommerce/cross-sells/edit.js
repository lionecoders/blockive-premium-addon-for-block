...
/**
 * WooCommerce cross-sell product ids can't be resolved from the generic
 * REST post shape used for editor previews, so this always renders static
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: `bpafb-tb-product-cross-sells bpafb-product-card-list bpafb-product-card-list--${ layout }`,
			...
			<div { ...blockProps }>
				{ Array.from( { length: Math.max( 1, previewCount ) } ).map( ( _, index ) => (
					<div className="bpafb-product-card" key={ index }>
						...
						<span className="bpafb-product-card-price">$39.99</span>
					</div>
				) ) }
			</div>
		</>
