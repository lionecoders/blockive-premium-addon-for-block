...
/**
 * WooCommerce related-product ids can't be resolved from the generic REST
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: `bpafb-tb-product-related bpafb-product-card-list bpafb-product-card-list--${ layout }`,
			...
			<div { ...blockProps }>
				{ Array.from( { length: Math.max( 1, previewCount ) } ).map( ( _, index ) => (
					<div className="bpafb-product-card" key={ index }>
						...
						<span className="bpafb-product-card-price">$49.99</span>
					</div>
				) ) }
			</div>
		</>
